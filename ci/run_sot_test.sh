...
function run_sot_test() {
    ...
    for file in ./test_*.py; do
        ...
        if [ -f "$file" ]; then
            ...
            echo Running:" STRICT_MODE=1 COST_MODEL=False MIN_GRAPH_SIZE=0 SOT_LOG_LEVEL=0 FLAGS_cudnn_deterministic=True SOT_ENABLE_STRICT_GUARD_CHECK=True python " $file
            # run unittests
