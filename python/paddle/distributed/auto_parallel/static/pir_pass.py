...
class P2PCommPass:
    ...
    @staticmethod
    def apply_p2p_comm_pass(dist_program, pipeline_strategy=None):
        pp_degree = 4
        if pipeline_strategy is not None:
            pp_degree = pipeline_strategy.pp_degree
        if pp_degree < 2:
            _logger.info(
                f"[liyamei debug] [apply_p2p_comm_pass] don't apply_p2p_comm_pass for pp_degree={pp_degree}"
            )
            return
        pp_stage = get_pp_stage_by_pp_degree(pp_degree)
        # if pp_stage % 2 != 0:
        #     _logger.info(
        #         f"[liyamei debug] [apply_p2p_comm_pass] don't apply_p2p_comm_pass for pp_stage={pp_stage} pp_degree={pp_degree}"
        #     )
        #     return
        last_fwd_op = None
        first_bwd_op = None
        ...
        if (
            last_fwd_op
            and first_bwd_op
            and last_fwd_op.name() == "pd_op.send_v2"
            and first_bwd_op.name() == "pd_op.recv_v2"
        ):
            first_bwd_op.op_role = OpRole.Forward
            ring_id = last_fwd_op.int_attr('ring_id')
            # first_bwd_op.set_execution_stream(f"send_stream_{ring_id}")
            _logger.info("[liyamei debug] [apply_p2p_comm_pass]")
