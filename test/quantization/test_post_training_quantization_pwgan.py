    ...
    def cache_unzipping(self, target_folder, zip_path):
        if not os.path.exists(target_folder):
            if zip_path.endswith('.rar'):
                cmd = f'mkdir {target_folder} && unrar x {zip_path} {target_folder}'
                print("1111111111111111111cmd:", cmd)
                os.system(cmd)
            elif zip_path.endswith('.zip'):
                cmd = f'mkdir {target_folder} && unzip {zip_path} -d {target_folder}'
                print("222222222222222222222cmd:", cmd)
                os.system(cmd)
            else:
                cmd = f'mkdir {target_folder} && tar xf {zip_path} -C {target_folder}'
                print("3333333333333333333333333:", cmd)
                os.system(cmd)
