...
class TestOpenVINOResnet50Model(unittest.TestCase):
    def setUp(self):
    ...
    def apply_test(self):
        first_config = self.get_config()
        start = time.time()
        self.paddle_predictor_openvino = paddle_infer.create_predictor(
            first_config
        )
        end1 = time.time()
        ...
        end2 = time.time()
        print("second load opt model time is: %.3f sec" % (end2 - end1))
