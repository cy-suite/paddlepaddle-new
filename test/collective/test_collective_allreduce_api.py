...
class TestCollectiveAllreduceAPI(TestDistBase):
    ...
    def test_allreduce_flagcx(self):
        if paddle.base.core.is_compiled_with_flagcx():
            self.check_with_place(
                "collective_allreduce_api.py", "allreduce", "flagcx"
            )

    def test_allreduce_flagcx_with_comm_context(self):
        dtypes_to_test = [
            "float16",
            "float32",
            "float64",
    ...
    def test_allreduce_flagcx_with_new_comm(self):
        dtypes_to_test = [
            "float16",
            "float32",
            "float64",
