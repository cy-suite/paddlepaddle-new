...
class TestDistShardingTraining(unittest.TestCase):
    ...
    def test_set_state_dict_with_fuse_optimizer_states_enabled(self):
        if not g_shard_enable_fuse_optimizer_states:
            print("skip fuse_optimizer_states enabled test.")
            return
        np_fc1 = np.random.random_sample((hidden_size, inner_size))
        np_fc2 = np.random.random_sample((inner_size, hidden_size))
        ...
        tmp_state_dict = copy.deepcopy(state_dict)
        dist_optimizer.set_state_dict(state_dict)
        dist_optimizer.fuse_optimizer_states()
        # check result
        other_param_names = [
            p_name
            for p_name in all_param_names
