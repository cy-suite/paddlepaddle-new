...
TEST_F(TestIndexExpr, TestCheckPattern) {
  ...
  EXPECT_TRUE(optim::CheckPattern(e, pattern, &map));
  map.clear();
  EXPECT_FALSE(optim::CheckPattern(e, pattern1, &map));
  map.clear();
  EXPECT_FALSE(optim::CheckPattern(e1, pattern, &map));
  map.clear();
  EXPECT_TRUE(optim::CheckPattern(e1, pattern1, &map));
}
}  // namespace common
}  // namespace cinn
