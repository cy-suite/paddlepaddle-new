  ...
  static List<DimExpr> SearchErasable(const List<DimExpr>& operands) {
    ...
    for (std::size_t i = 0; i < operands->size(); ++i) {
      ...
      for (std::size_t j = 0; j < operands->size(); ++j) {
        ...
        auto compare_j_i =
            symbol::EasyCompareGtOrGe(operands->at(j), operands->at(i));
  ...
  static List<DimExpr> SearchErasable(const List<DimExpr>& operands) {
    ...
    for (std::size_t i = 0; i < operands->size(); ++i) {
      ...
      for (std::size_t j = 0; j < operands->size(); ++j) {
        ...
        auto compare_i_j =
            symbol::EasyCompareGtOrGe(operands->at(i), operands->at(j));
  ...
  static List<DimExpr> SearchErasable(const List<DimExpr>& operands) {
    ...
    for (std::size_t i = 0; i < operands->size(); ++i) {
      ...
      for (std::size_t j = 0; j < operands->size(); ++j) {
        ...
        auto compare_j_i =
            symbol::EasyCompareGtOrGe(operands->at(j), operands->at(i));
        if (compare_j_i == DimExprCompareResult::GE) {
          is_redundant = true;
          break;
        }
      }
...
DimExpr Simplify(const DimExpr& expr) {
  ...
  for (bool keep_rewrite = true; keep_rewrite;) {
    ...
    DoPass<SimplifyMinWithGeOrGt>(&keep_rewrite, &ret);
    DoPass<SimplifyMaxWithGeOrGt>(&keep_rewrite, &ret);
