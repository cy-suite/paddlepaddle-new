...
#define CINN_WARP_SHUFFLE_INTERNAL_IMPL(REDUCE_TYPE, INITIAL_VALUE, DTYPE)                \
  __device__ inline DTYPE cinn_warp_shuffle_##REDUCE_TYPE##_internal(const DTYPE value) { \
    ...
    for (int offset = 16; offset > 0; offset /= 2) {                                      \
      DTYPE shfl_val = __shfl_down_sync(mask, tmp_val, offset, 32);                       \
      if (mask & (1 << (lane_id + offset))) {                                             \
        tmp_val = cinn_##REDUCE_TYPE(tmp_val, shfl_val);                                  \
      }                                                                                   \
    }                                                                                     \
...
#define CINN_BLOCK_REDUCE_MACRO(REDUCE_TYPE, INITIAL_VALUE, DTYPE) \
  __device__ inline DTYPE cinn_block_reduce_##REDUCE_TYPE(const DTYPE value, bool return_warp) { \
...
#define CINN_DISCRETE_REDUCE_SHM_IMPL(REDUCE_TYPE, value)                      \
...
#define CINN_DISCRETE_REDUCE_SHM_MACRO(REDUCE_TYPE, INITIAL_VALUE, DTYPE)      \
  __device__ inline DTYPE cinn_discrete_reduce_##REDUCE_TYPE##_shm(const DTYPE value, DTYPE* shm) { \
    CINN_DISCRETE_REDUCE_SHM_IMPL(REDUCE_TYPE, value);                         \
  }

EXPAND_REDUCE_INT32_MARCO(CINN_DISCRETE_REDUCE_SHM_MACRO)
EXPAND_REDUCE_INT64_MARCO(CINN_DISCRETE_REDUCE_SHM_MACRO)
EXPAND_REDUCE_FP32_MACRO(CINN_DISCRETE_REDUCE_SHM_MACRO)
EXPAND_REDUCE_FP64_MACRO(CINN_DISCRETE_REDUCE_SHM_MACRO)
EXPAND_REDUCE_BOOL_MACRO(CINN_DISCRETE_REDUCE_SHM_MACRO)

#ifdef CINN_CUDA_BF16
EXPAND_REDUCE_BF16_MACRO(CINN_DISCRETE_REDUCE_SHM_MACRO)
#endif

#ifdef CINN_CUDA_FP16
EXPAND_REDUCE_FP16_MACRO(CINN_DISCRETE_REDUCE_SHM_MACRO)
#endif

#undef CINN_DISCRETE_REDUCE_SHM_IMPL
#undef CINN_DISCRETE_REDUCE_SHM_MACRO
