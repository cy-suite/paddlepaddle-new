 ...
 *    not changed at all.
 * 2) Replace the `reduce_op` call with a simple `transformed_reduced_op`. for example:
 *  - welford: operator+ is implemented by C++ operator overloading. 
 *  - arg reduce: max, min op does not need to be replaced due to C++ operator overloading
