...
static const std::set<std::string> kExternFp64CallsCPU{
    "atan"};

void DealWithCpuIntrinsics(ir::Call *node, Expr *expr) {
  if (kExternFp32CallsCPU.count(node->name) || kExternFp64CallsCPU.count(node->name)) {
    PADDLE_ENFORCE_GE(
        node->read_args.size(),
        1UL,
    ...
    if (node->read_args.front().type().is_float(64)) {
      auto out_type = node->type();
      *expr = lang::CallExtern("cinn_host_" + node->name + "_fp64", node->read_args);
    }
  }
}
