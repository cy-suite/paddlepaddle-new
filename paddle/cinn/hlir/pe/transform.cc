  ...
  for (int i = 0; i < axes.size(); i++) {
    if (input_shape[axes[i]].is_constant() && new_starts[i].is_constant()) {
      ...
      if (new_starts[i].as_int64() < 0) {
        new_starts[i] = ir::Select::Make(
            ir::LT::Make(input_shape[axes[i]], -new_starts[i]),
            Expr(0),
            ir::Add::Make(input_shape[axes[i]], new_starts[i]));
      } else {
        new_starts[i] =
            ir::Select::Make(ir::LT::Make(input_shape[axes[i]], new_starts[i]),
                             input_shape[axes[i]],
                             new_starts[i]);
      }
