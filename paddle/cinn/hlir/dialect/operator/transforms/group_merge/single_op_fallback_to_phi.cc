...
class FusionOpPattern : public pir::OpRewritePattern<cinn::dialect::FusionOp> {
 public:
  ...
  bool MatchAndRewrite(cinn::dialect::FusionOp fusion_op,
    ...
    // Fallback only when FusionOp has two operators inside: AnySingleOp + yiled store
    // cf.yield
 ...
 private:
  ...
  pir::Operation* ReshapeOpPattern(
   ...
   pir::Operation* CastOpPattern(
      pir::Operation* op,
      pir::PatternRewriter& rewriter) const {  // NOLINT
    PADDLE_ENFORCE(op->isa<paddle::dialect::CastOp>(),
                   ::common::errors::InvalidArgument(
                       "Input should be paddle::dialect::CastOp, but got %s",
                       op->name()));
