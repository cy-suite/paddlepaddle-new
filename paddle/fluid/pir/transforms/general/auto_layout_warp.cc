...
#include "paddle/fluid/pir/transforms/general/auto_layout_warp.h"
...
namespace {
class AutoLayoutPassWarp : public pir::Pass {
 public:
  AutoLayoutPassWarp() : pir::Pass("auto_layout_pass_warp", 2) {}
  void Run(pir::Operation* op) override {
    ...
    pm.AddPass(pir::CreateAutoLayoutPass({"pd_op.fused_conv2d_add_act",
                                          "pd_op.conv2d",
                                          "pd_op.conv2d_transpose"}));
    pm.AddPass(pir::CreateAutoLayoutSimplifyPass());
    pm.Run(program_clone.get());

    if (IsNeedInsertTranspose(program_clone->module_op())) {
      pir::PassManager pm_(::pir::IrContext::Instance(), 2);
      pm_.AddPass(pir::CreateAutoLayoutPass({"pd_op.fused_conv2d_add_act",
                                             "pd_op.conv2d",
                                             "pd_op.conv2d_transpose"}));
  ...
  bool IsNeedInsertTranspose(pir::Operation* op) {
    ...
    VLOG(4) << "end IsNeedInsertTranspose"
...
std::unique_ptr<Pass> CreateAutoLayoutPassWarp() {
  return std::make_unique<AutoLayoutPassWarp>();
}

}  // namespace pir

REGISTER_IR_PASS(auto_layout_pass_warp, AutoLayoutPassWarp);
