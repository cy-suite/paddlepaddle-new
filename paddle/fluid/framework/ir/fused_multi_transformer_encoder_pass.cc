...
PDNode* MultiDevicesFusedMultiTransformerEncoderPattern::operator()() {
  ...
  auto* matmul_linear_out_var = pattern->NewNode(matmul_linear_out_repr())
                                    ->assert_is_op_output("matmul_v2")
                                    ->AsIntermediate()
                                    ->assert_is_op_input("all_reduce_sum");

  // communication all_reduce_sum
  auto* all_reduce_sum =
      pattern->NewNode(all_reduce_sum_repr())->assert_is_op("all_reduce_sum");
  auto* all_reduce_sum_out_var = pattern->NewNode(all_reduce_sum_out_repr())
                                     ->assert_is_op_output("all_reduce_sum")
                                     ->AsIntermediate()
                                     ->assert_is_op_input("elementwise_add");
  ...
  // communication all_reduce_sum
  auto* ffn_all_reduce_sum = pattern->NewNode(ffn_all_reduce_sum_repr())
