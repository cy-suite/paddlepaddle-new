...
PDNode* FusedAttentionGradPattern::operator()(PDNode* x,
  ...
  PDNode* mp_allreduce_out_node{nullptr};
  if (use_mp) {
    mp_allreduce_out_node =
        pattern->NewNode(c_identity_grad_out_repr())
            ->assert_is_op_output("all_reduce",
                                  "out",
                                  static_cast<int>(phi::ReduceType::kRedSum));
    auto* mp_allreduce_node =
        pattern->NewNode(c_identity_grad_op_repr())
            ->assert_is_op("all_reduce",
                           static_cast<int>(phi::ReduceType::kRedSum));
    fuse_qkv_matmul_grad_x_grad_node->assert_is_op_input(
