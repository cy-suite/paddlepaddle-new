...
class GradNodeIdentity : public GradNodeBase {
 public:
  // Constructor: configure fwd input tensors to grad node
  GradNodeIdentity(size_t bwd_in_slot_num, size_t bwd_out_slot_num)
      : GradNodeBase(bwd_in_slot_num, bwd_out_slot_num) {}
  ~GradNodeIdentity() override = default;
  ...
  std::string name() override { return "fw_primal node"; }
  // Members: define fwd input tensors
  // For Identity there is no fwd input tensor needed

  std::shared_ptr<GradNodeBase> Copy() const override {
    auto copied_node = std::make_shared<GradNodeIdentity>(*this);
    return copied_node;
  }
};
