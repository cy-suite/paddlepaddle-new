...
std::vector<paddle::Tensor> RunBackward(
  ...
  while (!queue.empty()) {
    ...
    try {
      ...
      paddle::framework::InsertCallStackInfo(
          node->name(), op_callstack_attr, &ex);
      LOG(WARNING) << "ERROR :" << node->name()
                   << "raises an EnforceNotMet exception in backward.cc"
                   << common::demangle(typeid(ex).name());
      throw ex;
    } catch (std::exception& ex) {
      LOG(WARNING) << "ERROR :" << node->name()
                   << "raises an exception in backward.cc"
                   << common::demangle(typeid(ex).name());
      throw ex;
    } catch (...) {
      LOG(WARNING) << "ERROR :" << node->name()
                   << "raises an unknown exception in backward.cc";
