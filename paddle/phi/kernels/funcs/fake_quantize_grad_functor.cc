...
void FakeQuantizeDequantizeGradLSQFunctor<Context, T>::operator()(
    ...
    trans(dev_ctx, 
          ...
          phi::funcs::LSQScaleGradFunctor<T>(bin_cnt, lsq_factor, inverse(scale_v), round_type)
    );

    DenseTensor scale_grad_elem_2;
    scale_grad_elem_2.Resize({x.dims()});
    dev_ctx.template Alloc<T>(
        &scale_grad_elem_2, scale_grad_elem_2.numel() * sizeof(T));
    
    MultiplyKernel<T, Context>(dev_ctx, out_grad, scale_grad_elem, &scale_grad_elem_2);
