...
struct EqualFunctor<phi::dtype::complex<T>, OutT> {
  HOSTDEVICE OutT operator()(const phi::dtype::complex<T>& a,
    ...
    if (isinf(static_cast<float>(a.real)) || isinf(static_cast<float>(a.imag)) ||
        isinf(static_cast<float>(b.real)) || isinf(static_cast<float>(b.imag))) {
      return static_cast<OutT>(a == b);
    }
    if (isnan(static_cast<float>(a.real)) || isnan(static_cast<float>(a.imag)) ||
        isnan(static_cast<float>(b.real)) || isnan(static_cast<float>(b.imag))) {
      return static_cast<OutT>(false);
    }
    float epsilon = 1e-8f;
