...
template <typename T>
__device__ __forceinline__ void ldmatrix_m8n8x4_trans_impl(uint32_t* R, T* smem_ptr) {
  uint32_t smem_int_ptr = static_cast<uint32_t>(__cvta_generic_to_shared(smem_ptr));
...
template <PrefetchMode prefetch_mode, SharedMemFillMode fill_mode, typename T>
__device__ __forceinline__ void pred_load_32b(T* smem_ptr, const T* gmem_ptr, bool predicate) {
  uint32_t smem_int_ptr = static_cast<uint32_t>(__cvta_generic_to_shared(smem_ptr));
  if constexpr (fill_mode == SharedMemFillMode::kFillZero) {
...
__device__ __forceinline__ void pred_load(T* smem_ptr, const T* gmem_ptr, bool predicate) {
  static_assert(num_bits == 128 || num_bits == 64 || num_bits == 32, "num_bits must be 128, 64 or 32.");
...
struct smem_t {
  ...
  static __device__ __forceinline__ uint32_t get_permuted_offset(uint32_t i, uint32_t j) {
    if constexpr (inv_stride <= 1) {
      ...
      return i / inv_stride * 8 + ((j + (i % inv_stride) * stride)) ^ ((i / inv_stride) % 8);
    }
  }
  ...
  static __device__ __forceinline__ uint32_t advance_offset_by_row(uint32_t offset) {
    if constexpr (row_stride == 2) {
      static_assert(step_size == 16 || step_size % 32 == 0, "Unsupported step size");
      ...
      static_assert(step_size == 8 || step_size % 16 == 0, "Unsupported step size");
